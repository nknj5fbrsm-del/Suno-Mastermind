...
interface StyleDisplayProps {
  data: GeneratedStyle;
  /** Wenn gesetzt: zwei Style-Varianten nebeneinander (wie Lyrics). */
...
/** Kompakte Anzeige: nur empfohlene Werte als 2 Icons + Zahl (keine Regler). */
...
/** Einzelne Style-Karte (Prompt + Wert-Pills + Referenzen + Insight). */
const StyleCard: React.FC<{
  ...
  variantLabel?: string;
  variantColor?: string;
}> = ({ data, editablePrompt, onPromptChange, variantLabel, variantColor }) => {
  const { tr } = useLang();
      ...
      <div className="flex-1 flex flex-col min-h-0 space-y-4">
        ...
        <div className="relative flex-shrink-0">
          ...
          <div className="relative">
            ...
            <textarea
              className={`w-full bg-white/5 dark:bg-black/20 rounded-xl pl-4 pr-2 py-2 text-[13px] font-black leading-relaxed outline-none resize-none h-24 transition-colors ${
          ...
          <div className="flex justify-between mt-1.5">
            <span className={`text-[10px] font-bold ${charCount > 120 ? 'text-red-500' : 'text-zinc-500 dark:text-zinc-400'}`}>{charCount} / 120</span>
            {isTooLong && <span className="text-[10px] font-black text-red-500">{tr.style.tooLong}</span>}
          </div>
        </div>
        {/* Empfohlene Werte: nur 2 Icons + Prozent */}
        ...
        {/* Referenzen (ähnliche Künstler) + Wirkung + Begründung */}
...
const StyleDisplay: React.FC<StyleDisplayProps> = ({ data, dataVariants, onRegenerate, onUpdatePrompt, onUpdatePromptVariant }) => {
  const { tr } = useLang();
    ...
    return (
      <section className="space-y-5 animate-fade-up">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <StyleCard data={dataVariants[0]} editablePrompt={editableVariant0} onPromptChange={handleVariant0Change} variantLabel={tr.lyrics.variant1} variantColor="text-suno-primary" />
          <StyleCard data={dataVariants[1]} editablePrompt={editableVariant1} onPromptChange={handleVariant1Change} variantLabel={tr.lyrics.variant2} variantColor="text-suno-secondary" />
        </div>
      </section>
    );
  }
        ...
        <div className="relative z-10 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className={`px-2.5 py-1 rounded-xl text-[9px] font-black border transition-all ${
              isTooLong ? 'bg-red-500/12 text-red-500 border-red-500/25 animate-pulse' : charCount > 100 ? 'bg-amber-500/12 text-amber-600 border-amber-500/25' : 'glass-btn text-zinc-500 border-white/0'
            }`}>
              {charCount} / 120
            </div>
          </div>
          <div className="relative">
            <div className={`absolute left-0 top-0 bottom-0 w-1 rounded-full transition-colors ${isTooLong ? 'bg-red-500' : 'bg-suno-primary'}`}></div>
            <textarea
              className={`w-full bg-transparent pl-4 pr-2 py-2 font-black text-lg md:text-xl leading-relaxed outline-none resize-none h-28 transition-colors ${isTooLong ? 'text-red-500' : 'text-zinc-900 dark:text-white'}`}
              value={editablePrompt}
